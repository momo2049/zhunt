...
if [ -d "$ZHUNT_DIR" ]; then
    ...
    cd "$ZHUNT_DIR" && unset http_proxy https_proxy HTTP_PROXY HTTPS_PROXY && git checkout -- app.py && git -c http.proxy= -c https.proxy= pull
else
...
# 装核心依赖
pip install --prefer-binary streamlit openai playwright beautifulsoup4 rich -q
